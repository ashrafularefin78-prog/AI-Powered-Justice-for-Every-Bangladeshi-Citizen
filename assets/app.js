...
/* ===== Monsoon of July - ambient memorial canvas (rain/smoke/embers) ===== */
function initMonsoonFX(){
  try{
    var cv=document.getElementById('monsoonFx');
    if(cv&&cv.dataset.fxInit)return;
    if(!cv){cv=document.createElement('canvas');cv.id='monsoonFx';}
    cv.dataset.fxInit='1';
    if(window.matchMedia&&matchMedia('(prefers-reduced-motion: reduce)').matches)return;
    var ctx=cv.getContext('2d');if(!ctx)return;
    var root=document.getElementById('pg-july')||document.body;
    var pageHero=root.querySelector?root.querySelector('.page-hero'):null;
    if(!pageHero)return;
    var dim=document.querySelector('.monsoon-dim');
    if(!cv.parentNode){
      cv.setAttribute('style','position:fixed;inset:0;width:100%;height:100%;z-index:1;pointer-events:none;opacity:0;transition:opacity 1.2s ease');
      (document.body||root).appendChild(cv);
      if(!dim){dim=document.createElement('div');dim.className='monsoon-dim';dim.setAttribute('aria-hidden','true');
        dim.setAttribute('style','position:fixed;inset:0;z-index:0;pointer-events:none;background:radial-gradient(ellipse 90% 70% at 50% 30%,transparent 40%,rgba(2,6,23,.55) 100%);opacity:0;transition:opacity 1.2s ease');
        (document.body||root).appendChild(dim);}
    }
    var W=0,H=0,dpr=Math.min(window.devicePixelRatio||1,2),running=false,raf=0;
    function size(){W=cv.width=Math.floor(innerWidth*dpr);H=cv.height=Math.floor(innerHeight*dpr);
      cv.style.width=innerWidth+'px';cv.style.height=innerHeight+'px';}
    size();addEventListener('resize',size);
    function R(a,b){return a+Math.random()*(b-a);}
    /* storm intensity model: 0 = gentle monsoon, 1 = full martyrs storm */
    var MAXD=220, intensity=0, target=0, bolt=0, boltNext=0, flashes=[],flNext=2,drips=[];
    /* peak-battle crest: entering the Abu Sayed card fires one synced peak -
       max storm intensity + denser gunfire for a few seconds, then eases back */
    var PEAK_RAMP_S=0.9,PEAK_HOLD_S=3.2,PEAK_EASE_S=2.6;
    var peakEl=null,peakOn=false,pkSeen=false,peakT=0,pLevel=0;
    function peakAnchor(){
      var c=document.querySelector('.martyr-strip-card');
      if(!c||!c.parentNode)return null;
      var cards=c.parentNode.querySelectorAll('.martyr-strip-card');
      for(var k=0;k<cards.length;k++){
        var im=cards[k].querySelector('img');
        if(im&&/abu sayed/i.test(im.getAttribute('alt')||''))return cards[k];
      }
      return c;
    }
    peakEl=peakAnchor();
    function peakZone(){
      if(!peakEl)return false;
      var r=peakEl.getBoundingClientRect();
      return r.top<innerHeight*0.85&&r.bottom>innerHeight*0.15;
    }
    var drops=[],smoke=[],embers=[];
    function mkDrop(storm){var s=storm?R(1.5,2.1):R(.55,1);
      return {x:R(-60,innerWidth+80),y:R(-innerHeight,innerHeight),l:R(9,26)*s,v:R(380,760)*s,w:R(.6,1.6),o:R(.2,.5)*s};}
    function seed(){
      drops=[];smoke=[];embers=[];
      var n=Math.min(MAXD,Math.floor(innerWidth/7));
      for(var i=0;i<n;i++)drops.push(mkDrop(false));
      for(var i2=0;i2<6;i2++)smoke.push({x:R(0,innerWidth),y:R(0,innerHeight),r:R(60,150),vx:R(-6,6),a:R(.02,.05)});
      for(var i3=0;i3<10;i3++)embers.push({x:R(0,innerWidth),y:R(0,innerHeight),r:R(.8,2.1),v:R(10,26),ph:R(0,6.28),a:R(.25,.6)});
    }
    seed();addEventListener('resize',function(){seed();});
    /* distant battle line: a per-load rooftop skyline. Flashes pin to
       rooflines (and occasionally street level between buildings) so the
       gunfire reads as one horizon of combat, not sparks in the sky. */
    var sky=[],ROOFY=0;
    function Rsd(a,b){return a+Math.random()*(b-a);}
    function buildSky(){
      sky=[];
      var x=-40;
      while(x<innerWidth+40){
        var w=Rsd(46,150),h=innerHeight*Rsd(0.06,0.185);
        sky.push({x:x,w:w,h:h});x+=w+Rsd(3,26);
      }
      ROOFY=innerHeight*0.78;
    }
    buildSky();
    addEventListener('resize',function(){buildSky();});
    function skySpot(ground){
      var b=sky[Math.floor(Math.random()*sky.length)]||{x:0,w:innerWidth,h:60};
      if(ground){return {x:b.x+Rsd(b.w*0.15,b.w*0.85),y:ROOFY+Rsd(24,64)};}
      return {x:b.x+Rsd(b.w*0.15,b.w*0.85),y:ROOFY-b.h-Rsd(-4,6)};
    }
    /* martyr zones: overlap of these sections with the viewport raises the storm */
    var zones=[];
    function findZones(){
      zones=[];
      var sel=['.martyrs-strip','#martyrs-list','#memorial','#victims-of-injustice','#mir-mugdho','#golam-nafiz','#farhan-faiyaaz','#wasim-akram','#abrar-fahad'];
      for(var z=0;z<sel.length;z++){
        var els=document.querySelectorAll(sel[z]);
        for(var e=0;e<els.length;e++){
          var el=els[e];
          if(el&&(root===document.body||root.contains(el)))zones.push(el);
        }
      }
    }
    findZones();
    addEventListener('resize',function(){findZones();});
    function inZone(){
      if(!zones.length)return false;
      for(var z=0;z<zones.length;z++){
        var r=zones[z].getBoundingClientRect();
        if(r.top<innerHeight*0.85&&r.bottom>innerHeight*0.15)return true;
      }
      return false;
    }
    var last=0;
    function frame(ts){
      ...
      /* ease intensity toward target (in-zone = storm) */
      target=inZone()?1:0;
      var pkz=peakZone();
      if(pkz){if(!pkSeen&&!peakOn){peakOn=true;peakT=0;}pkSeen=true;}
      else{pkSeen=false;}
      if(peakOn){
        peakT+=dt;
        pLevel=peakT<PEAK_RAMP_S?peakT/PEAK_RAMP_S:(peakT<PEAK_HOLD_S?1:Math.max(0,1-(peakT-PEAK_HOLD_S)/PEAK_EASE_S));
        if(peakT>=PEAK_HOLD_S+PEAK_EASE_S){peakOn=false;pLevel=0;}
      }
      if(pLevel>0)target=1;
      intensity+=(target-intensity)*Math.min(dt*1.6,1);
      if(intensity<.002)intensity=0;if(intensity>.998)intensity=1;
      var st=intensity;
      var bt=Math.max(0,(st-0.55)/0.45);/* blood tint 0..1 */
      try{window.__monsoonIntensity=st;RumbleFX.setIntensity(st);}catch(e){}
      window.__monsoonDbg=function(){return{st:intensity,fl:flashes.length,dr:drips.length,pk:pLevel,pkOn:peakOn,pkT:peakT,zn:zones.length,iz:inZone()?1:0,bld:sky.length,bolt:bolt,fys:flashes.map(function(f){return Math.round(f.y);}).slice(0,8)};}
      /* lightning flicker at full storm */
      if(st>.92){boltNext-=dt;if(boltNext<=0){bolt=1;boltNext=R(2.6,6.5);}}
      bolt=Math.max(0,bolt-dt*2.2);
      /* distant gunfire: muzzle flashes + tracers, cadence scales with storm */
      if(st>0.6){
        flNext-=dt;
        if(flNext<=0){
          flNext=R(0.7,2.2)/((0.5+0.5*st)*(1+3.5*pLevel));
          var sp=skySpot(Math.random()<0.18);
          flashes.push({x:sp.x,y:sp.y,ttl:0.11,max:0.11,r:R(26,64),tr:Math.random()<0.3?{x:sp.x,y:sp.y,vx:R(-1400,1400),vy:R(-260,260)*0.4,ttl:0.2}:null});
          try{RumbleFX.crack(sp.x/innerWidth);}catch(e){}
          var burst=pLevel>0.5?2:(pLevel>0.15?1:0);
          for(var b9=0;b9<burst;b9++){
            var bp=skySpot(Math.random()<0.12);
            flashes.push({x:bp.x,y:bp.y,ttl:0.11,max:0.11,r:R(26,64),tr:Math.random()<0.35?{x:bp.x,y:bp.y,vx:R(-1400,1400),vy:R(-260,260)*0.4,ttl:0.2}:null});
            try{RumbleFX.crack(bp.x/innerWidth);}catch(e2){}
          }
        }
      }
      ...
      var i,active=Math.round(drops.length*(0.35+0.65*st));
      /* storm haze behind everything */
      if(st>.05){
        var hg=ctx.createLinearGradient(0,0,0,innerHeight);
        hg.addColorStop(0,'rgba(2,6,23,'+(0.22*st)+')');
        hg.addColorStop(1,'rgba('+Math.round(24+52*bt)+',6,'+Math.round(6+10*bt)+','+(0.30*st)+')');
        ctx.fillStyle=hg;ctx.fillRect(0,0,innerWidth,innerHeight);
      }
      for(i=0;i<smoke.length;i++){var s0=smoke[i];s0.x+=(s0.vx*(1+1.4*st))*.016;if(s0.x<-s0.r)s0.x=innerWidth+s0.r;if(s0.x>innerWidth+s0.r)s0.x=-s0.r;
        var g=ctx.createRadialGradient(s0.x,s0.y,0,s0.x,s0.y,s0.r*(1+.5*st));
        g.addColorStop(0,'rgba(148,163,184,'+Math.min(s0.a*(1+1.8*st),.16)+')');g.addColorStop(1,'rgba(148,163,184,0)');
        ctx.fillStyle=g;ctx.beginPath();ctx.arc(s0.x,s0.y,s0.r,0,6.283);ctx.fill();}
      ctx.lineCap='round';
      for(i=0;i<active;i++){var d0=drops[i];d0.y+=d0.v*dt;d0.x-=d0.v*.12*dt;
        if(d0.y>innerHeight+30){d0.y=R(-80,-10);d0.x=R(-40,innerWidth+100);}
        if(d0.y<-90){d0.y=R(-60,-10);}
        var a=d0.o*(0.75+0.45*st);
        ctx.strokeStyle='rgba('+Math.round(190+30*bt)+','+Math.round(200-148*bt)+','+Math.round(212-152*bt)+','+a.toFixed(3)+')';
        ctx.lineWidth=d0.w;
        ctx.beginPath();
        ctx.moveTo(d0.x,d0.y);
        ctx.lineTo(d0.x-d0.l*.12,d0.y-d0.l);
        ctx.stroke();}
      ...
      if(st>0.7){while(drips.length<22)drips.push({x:R(0,innerWidth),y:R(-innerHeight,0),v:R(55,130),len:R(34,80),w:R(1.6,3.4),o:R(.45,.85)});}
      else if(st<0.5)drips=[];
      var da=Math.max(0,(st-0.7)/0.3);
      ...
      if(st<.6){
        var ea=1-st/.6;
        for(i=0;i<embers.length;i++){var e0=embers[i];e0.ph+=.05;e0.y-=e0.v*.016;e0.x+=Math.sin(e0.ph)*.4;
          if(e0.y<-12){e0.y=innerHeight+10;e0.x=R(0,innerWidth);}
          var tw=.5+.5*Math.sin(e0.ph*1.7);
          ctx.fillStyle='rgba(245,158,11,'+(e0.a*tw*.55*ea)+')';
          ctx.beginPath();ctx.arc(e0.x,e0.y,e0.r,0,6.283);ctx.fill();}
      }
      /* lightning: quick white wash + steel-blue glow */
      if(bolt>0){
        var bb=bolt*bolt;
        ctx.fillStyle='rgba(226,232,240,'+(0.16*bb)+')';
        ctx.fillRect(0,0,innerWidth,innerHeight);
        var lg=ctx.createRadialGradient(innerWidth*.5,innerHeight*.18,0,innerWidth*.5,innerHeight*.18,innerWidth*.75);
        lg.addColorStop(0,'rgba(191,219,254,'+(0.30*bb)+')');lg.addColorStop(1,'rgba(191,219,254,0)');
        ctx.fillStyle=lg;ctx.fillRect(0,0,innerWidth,innerHeight);
      }
      ctx.restore();
      /* vignette deepens with the storm */
      if(dim)dim.style.opacity=String(0.45+0.4*st);
    }
    /* gate: merged build observes the whole July band; standalone observes the
       hero AND every martyr zone so the storm (and the peak crest) live where
       the sections they belong to are, not just at the hero.
       Per-target tracking: one initial batch can mix intersecting and
       non-intersecting entries, so a plain +/- counter cancels itself out. */
    var liveMap={},liveTok=0;
    var io=new IntersectionObserver(function(en){
      for(var e1=0;e1<en.length;e1++){
        var e0=en[e1],el=e0.target;
        if(!el.__mtok)el.__mtok=++liveTok;
        if(e0.isIntersecting)liveMap[el.__mtok]=1;else delete liveMap[el.__mtok];
      }
      var any=false;for(var k9 in liveMap){any=true;break;}
      if(any){
        if(!running){running=true;last=0;raf=requestAnimationFrame(frame);cv.style.opacity='1';if(dim)dim.classList.add('on');}
      }else{
        if(running){running=false;cancelAnimationFrame(raf);cv.style.opacity='0';if(dim)dim.classList.remove('on');try{window.__monsoonIntensity=0;RumbleFX.setIntensity(0);}catch(e){}}
      }
    },{threshold:0,rootMargin:'12% 0px 12% 0px'});
    io.observe(root===document.body?pageHero:root);
    if(root===document.body){for(var zi9=0;zi9<zones.length;zi9++)io.observe(zones[zi9]);}
    document.addEventListener('visibilitychange',function(){
      if(document.hidden&&running){running=false;cancelAnimationFrame(raf);try{window.__monsoonIntensity=0;RumbleFX.setIntensity(0);}catch(e){}}
      else if(!document.hidden&&!running&&cv.style.opacity==='1'){running=true;last=0;raf=requestAnimationFrame(frame);}
    });
  }catch(e){}
}
/* ===== July monsoon rumble: opt-in low-frequency storm audio ================
   A single toggle (next to the sound toggle) starts a Web Audio graph:
   looped brown noise -> lowpass -> gain. The gain continuously follows the
   monsoon engine's storm intensity (0 = silence, 1 = full storm rumble).
   Off by default; nothing plays until the user clicks. */
var RumbleFX={on:false,ready:false};
(function(){
})();
function initRumbleFX(){
  try{
    /* only pages that actually carry the July monsoon get the toggle */
    if(!document.getElementById('pg-july')&&!document.querySelector('.martyrs-strip,#martyrs-list,#memorial'))return;
    var btn=document.createElement('button');
    btn.id='rumbleToggle';btn.type='button';btn.className='theme-toggle';
    btn.setAttribute('aria-label','Toggle July storm sound (rumble + distant gunfire)');
    btn.setAttribute('aria-pressed','false');
    btn.title='July storm sound: rain rumble + distant gunfire - click to enable';
    btn.innerHTML='<i class="fas fa-cloud-bolt" aria-hidden="true"></i>';
    var anchorBtn=document.getElementById('soundToggle')||document.getElementById('themeToggle');
    if(!anchorBtn||document.getElementById('rumbleToggle'))return;
    anchorBtn.insertAdjacentElement('afterend',btn);
    var styleEl=document.createElement('style');
    styleEl.textContent='#rumbleToggle.on{color:#93c5fd;border-color:rgba(147,197,253,.6);box-shadow:0 0 12px rgba(147,197,253,.35)}';
    document.head.appendChild(styleEl);
    var actx=null,gain=null,started=false,lmt=null;
    RumbleFX.setIntensity=function(v){
      try{
        if(!RumbleFX.on||!gain)return;
        var g=Math.pow(Math.max(0,Math.min(1,v)),1.5)*0.5;
        var t=actx.currentTime;
        gain.gain.cancelScheduledValues(t);
        gain.gain.setTargetAtTime(g,t,0.4);
      }catch(e){}
    };
    ...
    RumbleFX.crack=function(pan){
      try{
        ...
        var f=actx.createBiquadFilter();f.type='lowpass';f.frequency.value=1100;f.Q.value=0.7;
        var g=actx.createGain();g.gain.value=0.34+Math.random()*0.16;
        src.connect(f);f.connect(g);
        var p=actx.createStereoPanner?actx.createStereoPanner():null;
        if(p){p.pan.value=Math.max(-1,Math.min(1,((pan||0.5)-0.5)*1.2));g.connect(p);p.connect(lmt||actx.destination);}
        else{g.connect(lmt||actx.destination);}
        src.start(t);
        ...
        og.gain.setValueAtTime(0.42,t);og.gain.exponentialRampToValueAtTime(0.001,t+0.18);
        o.connect(og);og.connect(lmt||actx.destination);o.start(t);o.stop(t+0.2);
      }catch(e){}
    };
    function build(){
      ...
      if(!AC)return false;
      actx=new AC();
      var len=5*actx.sampleRate,buf=actx.createBuffer(1,len,actx.sampleRate),ch=buf.getChannelData(0);
      var lp=0;
      for(var i=0;i<len;i++){var w=Math.random()*2-1;lp=(lp+0.02*w)/1.02;ch[i]=lp*3.5;}
      var src=actx.createBufferSource();
      src.buffer=buf;src.loop=true;src.loopStart=0;src.loopEnd=6;
      var filt=actx.createBiquadFilter();filt.type='lowpass';filt.frequency.value=120;filt.Q.value=0.5;
      gain=actx.createGain();gain.gain.value=0;
      lmt=actx.createDynamicsCompressor();
      lmt.threshold.value=-12;lmt.knee.value=20;lmt.ratio.value=8;lmt.attack.value=0.002;lmt.release.value=0.18;
      src.connect(filt);filt.connect(gain);gain.connect(lmt);lmt.connect(actx.destination);
      src.start();
      return true;
    }
    function startAudio(){
      ...
      RumbleFX.setIntensity(window.__monsoonIntensity||0);
      return true;
    }
    function stopAudio(){
      try{
        if(gain&&actx){var t=actx.currentTime;gain.gain.cancelScheduledValues(t);gain.gain.setTargetAtTime(0,t,0.2);}
        if(actx&&actx.state==='running'){setTimeout(function(){try{if(!RumbleFX.on&&actx){actx.suspend();}}catch(e){}},700);}
      }catch(e){}
    }
    btn.addEventListener('click',function(){
      RumbleFX.on=!RumbleFX.on;
      if(RumbleFX.on){
        if(startAudio()){
          btn.classList.add('on');
          btn.setAttribute('aria-pressed','true');
          btn.title='July storm sound playing - click to stop';
          btn.innerHTML='<i class="fas fa-volume-off" aria-hidden="true"></i>';
        }else{
          RumbleFX.on=false;
    ...
    document.addEventListener('visibilitychange',function(){
      try{
        ...
        else if(RumbleFX.on){actx.resume();}
      }catch(e){}
    });
    RumbleFX.ready=true;
  }catch(e){}
}
if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',initRumbleFX);}else{initRumbleFX();}
/* ===== Cinematic scroll reveals: July timeline + martyr cards ==============
   Elements start as .cin (dim, low, blurred - "inside the rain") and rise to
   .cin-done as they enter the viewport, with a small sibling stagger per
   container. Classes are added by JS only, so no-JS renders fully visible.
   Scoped to the July page; disabled under prefers-reduced-motion. */
function initCinematicFX(){
  try{
    if(window.matchMedia&&matchMedia('(prefers-reduced-motion: reduce)').matches)return;
    var inJuly=!!document.getElementById('pg-july')||/(^|\/)july\.html?$/i.test(location.pathname||'');
    if(!inJuly)return;
    var sel='.timeline-item,.martyr-strip-card,.martyr-card';
    var els=document.querySelectorAll(sel);
    if(!els.length)return;
    /* mark + group by parent container for the stagger index */
    var groups={};
    for(var i=0;i<els.length;i++){
      var el=els[i];
      if(el.dataset.cinInit)continue;
      el.dataset.cinInit='1';
      el.classList.add('cin');
      var p=el.parentNode;
      var cls=(p&&p.className&&p.className.toString?p.className.toString():'').trim();
      var gk=cls?('.'+cls.split(/\s+/).join('.')):'#doc';
      (groups[gk]=groups[gk]||[]).push(el);
    }
    for(var k in groups){
      var arr=groups[k];
      for(var j=0;j<arr.length;j++)arr[j].style.setProperty('--cin-d',Math.min((j%8)*80,560)+'ms');
    }
    var cio=new IntersectionObserver(function(en){
      en.forEach(function(e0){
        if(e0.isIntersecting){
          e0.target.classList.add('cin-done');
          cio.unobserve(e0.target);
        }
      });
    },{threshold:0.12,rootMargin:'0px 0px -8% 0px'});
    for(var q=0;q<els.length;q++)cio.observe(els[q]);
  }catch(e){}
}
if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',initCinematicFX);}else{initCinematicFX();}
if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',initMonsoonFX);}else{initMonsoonFX();}


/* ===== FIGURES registry: add a person with one data entry, zero code edits =====
   Entry shape:
   FIGURES.push({
     id:'sayeedi',                       // unique slug
     match:['sayeedi','sayedee','...'],  // lowercase EN/BN substrings for the photo card
     keys:['sayeedi_bio','sayeedi_trial'],// cR keys that always show the portrait
     photo:{ src:'assets/img/sayeedi.png',
             name:'Delwar Hossain Sayeedi',
             bn:'দেলাওয়ার হোসাইন সাঈদী',
             dates:'1940-2023',
             credit:'Photo: Wikipedia (fair use - educational)',
             link:'https://en.wikipedia.org/wiki/File:Sayeedi.png' },
     entries:[ {key:'sayeedi_bio',   text:'EN text | BN text'},
               {key:'sayeedi_trial', text:'EN text | BN text'},
               {key:'sayeedi_books', text:'EN text | BN text'} ],
     aliases:[ ['delwar hossain sayeedi','sayeedi_bio'], ['sayeedi','sayeedi_bio'], ... ]
   });
   installFigures() runs once at load: entries -> cR, aliases -> aliasMap.
   martyrPhotoHTML consults figurePhotoHTML() so portraits need no new branch.
*/
var FIGURES=[];
function figurePhotoHTML(k,m){
  function ev(){return m.indexOf('kill')>-1||m.indexOf('killed')>-1||m.indexOf('shot')>-1||m.indexOf('shooting')>-1||m.indexOf('death')>-1||m.indexOf('died')>-1||m.indexOf('murder')>-1||m.indexOf('assassin')>-1||m.indexOf('attack')>-1||m.indexOf('funeral')>-1||m.indexOf('janaza')>-1||m.indexOf('mourn')>-1||m.indexOf('protest')>-1||m.indexOf('grave')>-1||m.indexOf('হত্যা')>-1||m.indexOf('নিহত')>-1||m.indexOf('হত্যাকাণ্ড')>-1||m.indexOf('খুন')>-1||m.indexOf('গুলি')>-1||m.indexOf('মৃত্যু')>-1||m.indexOf('নিধন')>-1||m.indexOf('শোক')>-1||m.indexOf('জানাজা')>-1||m.indexOf('প্রতিবাদ')>-1||m.indexOf('কবর')>-1;}
  for(var f=0;f<FIGURES.length;f++){
    var F=FIGURES[f],i,hit=false;
    for(i=0;i<F.keys.length;i++){if(k===F.keys[i]){hit=true;break;}}
    if(!hit&&F.keyPrefixes){for(i=0;i<F.keyPrefixes.length;i++){var px=F.keyPrefixes[i];if(k===px||k.indexOf(px)===0){hit=true;break;}}}
    if(!hit&&F.match){for(i=0;i<F.match.length;i++){if(m.indexOf(F.match[i])>-1){hit=true;break;}}}
    if(hit){
      var p=F.photo,col=F.color||'16,185,129';
      var sh='<div style="max-width:200px;margin:0 0 12px;">';
      sh+='<div style="border-radius:12px;overflow:hidden;border:2px solid rgba('+col+',.45);box-shadow:0 4px 18px rgba(0,0,0,.25);background:#fff"><img src="'+p.src+'" alt="'+p.name+' ('+p.dates+')" style="width:100%;height:auto;display:block" loading="lazy"></div>';
      sh+='<div style="font-size:10px;color:rgba(255,255,255,.7);text-align:center;margin-top:6px;font-weight:600;">'+p.name+'</div>';
      sh+='<div style="font-size:9px;color:rgba(255,255,255,.45);text-align:center;">'+p.bn+' &middot; '+(F.sub||p.dates)+'</div>';
      sh+='<div style="font-size:9px;color:rgba(255,255,255,.42);text-align:center;margin-top:4px;"><a href="'+p.link+'" target="_blank" rel="noopener" style="color:rgba(255,255,255,.65);text-decoration:none;border-bottom:1px dotted rgba(255,255,255,.3);">'+p.credit+'</a></div>';
      if(F.extras&&ev()){
        for(var x=0;x<F.extras.length;x++){
          var E=F.extras[x];
          if(E.grid){
            sh+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:6px;">';
            sh+='<img src="'+E.grid[0]+'" alt="'+(E.alts?E.alts[0]:'')+'" style="width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:8px;border:1px solid rgba('+col+',.3);display:block" loading="lazy">';
            sh+='<img src="'+E.grid[1]+'" alt="'+(E.alts?E.alts[1]:'')+'" style="width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:8px;border:1px solid rgba('+col+',.3);display:block" loading="lazy">';
            sh+='</div>';
          }else if(E.img){
            sh+='<div style="margin-top:6px;border-radius:8px;overflow:hidden;border:1px solid rgba('+col+',.3);background:#fff"><img src="'+E.img+'" alt="'+(E.alt||'')+'" style="width:100%;'+(E.ar?'aspect-ratio:'+E.ar+';':'')+'object-fit:cover;display:block" loading="lazy"></div>';
          }
          if(E.caption)sh+='<div style="font-size:9.5px;color:rgba(255,255,255,.5);line-height:1.5;margin-top:4px;">'+E.caption+'</div>';
          if(E.link)sh+='<div style="font-size:9.5px;color:rgba(255,255,255,.42);line-height:1.5;"><a href="'+E.link+'" target="_blank" rel="noopener" style="color:rgba(255,255,255,.65);text-decoration:none;border-bottom:1px dotted rgba(255,255,255,.3);">'+(E.credit||E.caption)+'</a></div>';
        }
      }
      if(F.note)sh+='<div style="font-size:9px;color:rgba(255,255,255,.35);line-height:1.5;margin-top:5px;">'+F.note+'</div>';
      if(F.cta)sh+='<a href="'+F.cta.href+'" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:8px;width:100%;margin-top:8px;box-sizing:border-box;background:rgba('+col+',.1);border:1px solid rgba('+col+',.4);color:#ffb09c;border-radius:10px;padding:8px 12px;font-size:12px;font-weight:600;text-decoration:none;">'+F.cta.label+' &rarr;</a>';
      sh+='</div>';
      return sh;
    }
  }
...
function installFigures(){
  for(var f=0;f<FIGURES.length;f++){
    var F=FIGURES[f],i;
    for(i=0;i<F.entries.length;i++){try{cR[F.entries[i].key]=F.entries[i].text;}catch(e){}}
    for(i=0;i<F.aliases.length;i++){try{aliasMap[F.aliases[i][0]]=F.aliases[i][1];}catch(e){}}
  }
}

FIGURES.push({
  id:'sayeedi',
  match:['sayeedi','sayedee','delwar','সাঈদী','দেলাওয়ার'],
  keys:['sayeedi_bio','sayeedi_trial','sayeedi_books'],
  photo:{ src:'assets/img/sayeedi.png',
          name:'Delwar Hossain Sayeedi',
          bn:'দেলাওয়ার হোসাইন সাঈদী',
          dates:'1940-2023',
...
function martyrPhotoHTML(mk,msg){
  try{
    ...
    /* generic registry-driven portrait card (all figures live in FIGURES) */
    var card=figurePhotoHTML(k,m);
    if(!card){
      /* container-style questions ("who are the july martyrs") -> strip card */
      var martyrWords=(m.indexOf('july martyr')>-1||m.indexOf('martyrs of july')>-1||m.indexOf('martyrs list')>-1||m.indexOf('জুলাই শহীদ')>-1||m.indexOf('শহীদদের')>-1);
      var stripKeys=(k==='july_martyrs'||k==='gen z july martyrs'||k==='july_overview'||k==='grokipedia_overview');
      if(martyrWords&&stripKeys){
        var s='<div style="max-width:320px;margin:0 0 12px;">';
        s+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">';
        s+='<a href="assets/img/abu-sayed-iconic.jpg" style="display:block;text-decoration:none;"><div style="border-radius:12px;overflow:hidden;border:2px solid rgba(245,158,11,.5);background:#000"><img src="assets/img/abu-sayed-iconic.jpg" alt="Abu Sayed, first martyr of the July Uprising" style="width:100%;aspect-ratio:3/4;object-fit:cover;display:block" loading="lazy"></div><div style="text-align:center;font-size:10.5px;color:rgba(255,255,255,.6);margin-top:5px;line-height:1.4;">Abu Sayed<br><span style="color:rgba(255,255,255,.35);">&#x09AA;&#x09CD;&#x09B0;&#x09A5;&#x09AE; &#x09B6;&#x09B9;&#x09C0;&#x09A6;, 2024</span></div></a>';
        s+='<a href="assets/img/mir-mugdho-memorial.jpg" style="display:block;text-decoration:none;"><div style="border-radius:12px;overflow:hidden;border:2px solid rgba(16,185,129,.5);background:#fff"><img src="assets/img/mir-mugdho-memorial.jpg" alt="Mir Mugdho memorial" style="width:100%;aspect-ratio:3/4;object-fit:cover;display:block" loading="lazy"></div><div style="text-align:center;font-size:10.5px;color:rgba(255,255,255,.6);margin-top:5px;line-height:1.4;">Mir Mugdho<br><span style="color:rgba(255,255,255,.35);">&#x09AE;&#x09C0;&#x09B0; &#x09AE;&#x09C1;&#x0997;&#x09CD;&#x09A7;, 2024</span></div></a>';
        s+='<div style="font-size:9.5px;color:rgba(255,255,255,.4);line-height:1.5;margin-top:6px;text-align:center;"><a href="https://commons.wikimedia.org/wiki/File:Abu_Saeed_was_standing_in_front_of_the_gun_just_before_his_murder..jpg" target="_blank" rel="noopener" style="color:rgba(255,255,255,.65);text-decoration:none;border-bottom:1px dotted rgba(255,255,255,.3);">Photos: Chakraborty &middot; Tamim &middot; CC BY-SA 4.0</a></div>';
        s+='</div>';
        return s;
      }
      return '';
...
function showToast(msg,kind){
  ...
  t.textContent=msg;
  t.className='auth-toast show'+(kind==='ok'?' toast-ok':' toast-err');
...
function successGo(sec){
  ...
  else{showToast('Opening dashboard...','ok');}
}
...
var divColors={
'Rangpur':'#22c55e','Rajshahi':'#ef4444','Mymensingh':'#10b981',
